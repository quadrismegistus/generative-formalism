...
def get_legacy_df_poems1(path_pkl=PATH_RAW_PKL, verbose=True):
    if verbose:
...
def get_legacy_df_poems2(path_json=PATH_RAW_JSON, verbose=True):
...
def get_stash_df_poems(verbose=True):
    if verbose:
        print(f"  * Collecting from {STASH_GENAI_RHYME_PROMPTS.path}")
    odf = STASH_GENAI_RHYME_PROMPTS.df.rename(columns={"_value": "response"}).drop(
...
def preprocess_rhyme_promptings(overwrite=False, save_to=PATH_GENAI_PROMPTS_IN_PAPER, verbose=True, **kwargs):
...
def get_genai_rhyme_promptings_as_in_paper(
    *args, overwrite=False, save_to=PATH_GENAI_PROMPTS_IN_PAPER, verbose=True, **kwargs
):
    """
...
def get_genai_rhyme_promptings_as_replicated(*args, verbose=True, **kwargs):
    """
