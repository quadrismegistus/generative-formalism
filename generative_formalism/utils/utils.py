...
def timeout(seconds):
    ...
    Args:
        seconds (int/float): Maximum seconds to allow function to run
        
    Returns:
        Function decorator that returns None if timeout occurs or any exception is raised
    ...
    Example:
        ...
        result = slow_function()  # Returns None after 5 seconds
    """
    import signal
    import functools
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                # Reset alarm and handler on any exception
                try:
                    signal.alarm(0)
                    signal.signal(signal.SIGALRM, old_handler)
                except:
